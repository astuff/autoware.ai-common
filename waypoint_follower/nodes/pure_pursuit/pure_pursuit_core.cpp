...
double PurePursuitNode::computeCommandAccel() const
{
  ...
  double v_i = current_linear_velocity_;
  double v_f = computeCommandVelocity();
  return (v_f * v_f - v_i * v_i) / (2 * delta_d);
}

// Have no clue why this is needed
// v^2 * kappa is the normal component of the acceleration vector. Why divide by gravity?
double PurePursuitNode::computeAngularGravity(double velocity, double kappa) const
{
  const double gravity = 9.80665;
  return (velocity * velocity * kappa) / gravity;
}
